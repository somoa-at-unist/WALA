...
//        } catch (ClassCastException ex) {}
    ...
    @Fuzz
    public void testWith3(@InRange(minInt=1999, maxInt=2009) int year,
                          @InRange(minInt=1, maxInt=12, isFixed=true) int month,
                          @InRange(minInt=1, maxInt=30, isFixed=true) int day,
                          @InRange(minInt=1, maxInt=12, isFixed=true) int hour1,
        ...
        Log.infoLog("testWith3 is called");
        DateTimeFieldType time = null;
        int value = 0;
        switch(z) {
            case 1:
                ...
                value = year;
            case 2:
                time = DateTimeFieldType.hourOfDay();
                value = hour2;
            case 3:
        ...
        Partial updated = null;
        try {
            // check whether we expect an exception
            for (DateTimeFieldType t: types) {
                if (t.getRangeDurationType().equals(time.getRangeDurationType())) {
                    expectException = true;
                }
            }
            updated = test.with(time, value);
            Log.infoLog("with is returned");
        } catch (IllegalArgumentException ex) {
            Log.infoLog("IllegalArgumentException occurred");
            boolean result = check2(test, hour1, min1);
            String out = "null";
            if (updated != null) out = updated.toString();
            final String out2 = out;
            Log.logOutIf(result && expectException, () -> new String[]{ out2 });
            return;
        }
        Log.infoLog("An exception did not occur");
        boolean result = check2(test, hour1, min1);
        String out = "null";
        if (updated != null) out = updated.toString();
        final String out2 = out;
    ...
    private boolean check2(Partial test, int hour, int min) {
        return hour == test.get(DateTimeFieldType.hourOfDay()) &&
