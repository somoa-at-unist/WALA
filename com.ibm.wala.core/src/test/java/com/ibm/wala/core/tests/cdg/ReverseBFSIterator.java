 ...
 */
public class ReverseBFSIterator<T> implements Iterator<T> {
   ...
   */
  public ReverseBFSIterator(Graph<T> G, T N) {
    if (G == null) {
   ...
   * @throws IllegalArgumentException if G is null
   */
  public ReverseBFSIterator(Graph<T> G, Iterator<? extends T> nodes) {
    if (G == null) {
      throw new IllegalArgumentException("G is null");
    }
    if (nodes == null) {
   ...
   */
  public ReverseBFSIterator(Graph<T> G) throws NullPointerException {
  ...
  private void visitParent(T N) {
    for (T parent : Iterator2Iterable.make(getConnected(N))) {
