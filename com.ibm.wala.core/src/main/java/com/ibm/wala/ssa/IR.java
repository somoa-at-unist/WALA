 ...
 * An SSA IR.
 *
 * <p>The IR (Intermediate Representation) is the central data structure that represents the
 * instructions of a particular method. The IR represents a method's instructions in a language
 * close to JVM bytecode, but in an SSA-based register transfer language which eliminates the stack
...
public abstract class IR implements IRView {
  ...
  /**
   * @return a String which is a readable representation of the instruction position corresponding
            ...
            for (SSAIndirectionData.Name n : names) {
              if (getIndirectionData().getUse(j, n) != -1) {
                result
                    ...
                    .append(n)
                    .append(" -> ")
                    .append(getIndirectionData().getUse(j, n));
                any = true;
              }
            }
            if (any) {
            ...
            for (SSAIndirectionData.Name n : names) {
              if (getIndirectionData().getDef(j, n) != -1) {
                result
                    ...
                    .append(n)
                    .append(" <- ")
                    .append(getIndirectionData().getDef(j, n));
                any = true;
              }
            }
            if (any) {
  ...
  /**
   * Returns the normal instructions. Does not include {@link SSAPhiInstruction}, {@link
   * SSAPiInstruction}, or {@link SSAGetCaughtExceptionInstruction}s, which are currently managed by
   * {@link BasicBlock}. Entries in the returned array might be null.
   *
   * <p>This may go away someday.
   */
  @Override
   ...
   * over the {@link BasicBlock}s, one at a time.
   *
   * <p>TODO: this looks buggy to me .. looks like it's hardcoded for Phis. Does it work for Pis?
   */
  ...
  /**
   * Get the {@link TypeReference} that describes the ith parameter to this method. By convention,
   * for a non-static method, the 0th parameter is "this".
   */
  ...
  /**
   * @return an {@link Iterator} of all the allocation sites ( {@link NewSiteReference}s ) in this
   *     IR
   */
  @Override
   ...
   *
   * <p>Be very careful; note the strange identity semantics of SSAInstruction, using ==. You can't
   * mix SSAInstructions and IRs freely.
   */
  ...
  public boolean isEmptyIR() {
    ...
    for (SSAInstruction instruction : instructions) if (instruction != null) return false;
   ...
   * @return if we know that immediately after the given program counter, v_vn corresponds to one or
   *     more locals and local variable names are available, the name of the locals which v_vn
  ...
  public interface SSA2LocalMap {
     ...
     * @param index an index into the IR instruction array
     * @param vn a value number
     * @return if we know that immediately after the given program counter, v_vn corresponds to one
     *     or more locals and local variable names are available, the name of the locals which v_vn
     *     represents. Otherwise, null.
     */
